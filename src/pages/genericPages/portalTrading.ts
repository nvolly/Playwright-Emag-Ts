...
export default class PortalTradingPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.complexConditionat = page.locator('div[valoare="P"][id="tabConditionat"]');
    ...
    this.functiaPublicaDetinutaAD = page.locator('input#functiaPublicaDetinuta');
    ...
    this.butonInchideDetaliiOrdin = page.locator('button#butonInchideDetaliiOrdin');
    ...
    this.buttonSubcontFaraParola = page.locator('button#buttonSubcontFaraParola');
    ...
    this.butonCumparaSimbolNormal = page.locator('button#butonCumparaSimbolNormal');
    ...
    this.campDataConditieOrdinNou = page.locator('input#campDataConditieOrdinNou');
    ...
    this.campPretModificareOrdin = page.locator(this.campPretModificareOrdinSelector);

    this.campCantitateModificareOrdinSelector = 'input#campCantitateModificareOrdin';
    this.campCantitateModificareOrdin = page.locator(this.campCantitateModificareOrdinSelector);

    this.campPretStopModificareOrdinSelector = 'input#campPretStopModificareOrdin';
    this.campPretStopModificareOrdin = page.locator(this.campPretStopModificareOrdinSelector);

    this.campConditieDataModificareOrdinSelector = 'input#campConditieDataModificareOrdin';
    ...
    this.butonAnulareOrdinFonduri = page.locator('button#butonAnulareOrdinFonduri');
    ...
    this.istoricTransferuriButonAnulare = page.locator('button#istoricTransferuriButonAnulare');
    this.istoricTransferuriAnulare = page.locator('button#istoricTransferuriAnulare');
    ...
    this.retrageriAdaugcontnouInformatii = page.locator('div#retrageriAdaugcontnouInformatii');
    this.retrageriUploadExtrasdecont = page.locator('input#retrageriUploadExtrasdecont');
    ...
    this.retrageriOperatiuniInformatii = page.locator('div#retrageriOperatiuniInformatii');
  ...
  beneficiarRealAD(option: 'da' | 'nu'): Locator {
    return this.page.locator(`.rboption-danu label:has-text("${option}") input[type="radio"]`);
  }
  ...
  cetateanSUA(option: 0 | 1): Locator {
    return this.page.locator(`.rboption-danu label[for="persSUA_${option}"] input[type="radio"]`);
  }
  ...
  async gotoSimbol(simbol = 'TLV'): Promise<void> {
    ...
    const expectedTitle =
      process.env.ENV === 'beta' ? 'webdevS - Portal Trading' : 'Portal Trading';
    expect(await this.pageTitle.textContent()).toBe(expectedTitle);

    await this.page.addLocatorHandler(this.page.locator('div.header_overlay'), async () => {
  ...
  async gotoDirect(path: string): Promise<void> {
    ...
    if (await this.page.locator('form.login_form').isVisible()) {
      ...
      await this.page.evaluate((): void => {
        localStorage.setItem('testing_auto', '1');
      });
      await this.page.context().storageState({ path: 'state.json' });
    }
    ...
    await this.page.addLocatorHandler(
      ...
      async (): Promise<void> => {
        await this.page.evaluate((): void => removeHeaderOverlay());
      }
    );
